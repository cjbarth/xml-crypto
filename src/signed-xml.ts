...
export class SignedXml {
  ...
  loadSignature(signatureNode: Node | string): void {
    ...
    if (isDomNode.isAttributeNode(node)) {
      this.canonicalizationAlgorithm = utils.collapseWhitespace(
        node.value,
    ...
    if (isDomNode.isAttributeNode(signatureAlgorithm)) {
      this.signatureAlgorithm = utils.collapseWhitespace(
  ...
  private loadReference(refNode: Node) {
    ...
    const digestAlgo = utils.collapseWhitespace(attr.value);
    ...
    if (nodes.length !== 0) {
      ...
      for (const transform of transformsAll) {
        ...
        if (transformAttr) {
          transforms.push(utils.collapseWhitespace(transformAttr.value));
        }
      }
